...
import { hlsPlaylistHeadResponse, hlsPlaylistResponse } from "$lib/server/transcoding/hls";
...
export const GET: RequestHandler = async ({ params, locals, url, request }) => {
  ...
  // Production callers always read the on-disk master.m3u8, which carries
  // accurate per-segment durations (keyframe-derived for remux when the source
  // exposes cheap keyframes, or the 16s fixed grid for transcode / fallback
  // sources). hlsPlaylistResponse rewrites segment URIs through the signed
  // `segments/` route.
  ...
  try {
    ...
    const revalidated = await currentUnchangedPlayableHlsArtifact({
      sessionId: params.sessionId,
      userId: auth.userId,
      playlistPath: artifact.playlistPath,
      artifact: "playlist",
    });
    if (revalidated instanceof Response) return withSignedPlaybackHeaders(revalidated, auth.signed);
    if (request?.signal?.aborted) return withSignedPlaybackHeaders(cancelledPlaylistResponse(), auth.signed);
