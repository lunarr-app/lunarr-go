...
// The MovieWithFileFunc type is an adapter to allow the use of ordinary
// function as MovieWithFile mutator.
type MovieWithFileFunc func(context.Context, *ent.MovieWithFileMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MovieWithFileFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MovieWithFileMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MovieWithFileMutation", m)
}
