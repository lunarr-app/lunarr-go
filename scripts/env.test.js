...
describe("startup env loader", () => {
  test("parses dotenv lines used by Lunarr start", () => {
    expect(parseDotenvLine("AUTH_SECRET=abc")).toEqual({ key: "AUTH_SECRET", value: "abc" });
    expect(parseDotenvLine("export PORT=3000")).toEqual({ key: "PORT", value: "3000" });
    expect(parseDotenvLine("ORIGIN='http://127.0.0.1:3000'")).toEqual({ key: "ORIGIN", value: "http://127.0.0.1:3000" });
